...
Add this block of code in the same index.tsx file.
...
We fetched the posts from the API and this is what the structure looks like
...
For the details page, `post/[slug].tsx`, we’ll get the slug from the route’s param and query our CMS API for the single post's details. We'll be bringing in `getStaticProps` again