...
To pre-render all the available post, it is important we provide a list of all the post slug. To do this, we will export an async function `getStaticPath` that returns an object in the `posts/[slug].tsx` file
...
If you already have surge CLI already, then run the legendary command 
...
Static Site Generation is a good approach where you have a definite number of content just like a blog.