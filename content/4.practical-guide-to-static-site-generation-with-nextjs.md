...
We fetched the posts from our CMS API and this is what the structure looks like
...
For the details page, `post/[slug].tsx`, we’ll get the slug from the route’s param and query our CMS API for the single post's details. We'll be bringing in `getStaticProps` again. Before jumping in, we'll create some helper functions to avoid redundant repition, When calling our CMS API. here's what it looks like
...
And then back to our `posts/[slug].tsx` we'll using our helper function to fecth the post by `slug`
